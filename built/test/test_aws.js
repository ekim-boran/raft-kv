...
var params = {
    security_group: "sg-067af7bc00c024638",
    key_name: "raft_test",
...
const startServers = async (n) => {
    ...
    for (let i of response.Instances)
        await waitUntilInstanceRunning({ client: ec2 }, { InstanceIds: [i.InstanceId] });
};
const getRunningInstances = async () => {
    const response = await ec2.send(new DescribeInstancesCommand({}));
    const names = response.Reservations.flatMap(r => r.Instances).filter(i => i.State.Name == "running").map(i => ({ public_dns: i.PublicDnsName, private_ip: i.PrivateIpAddress, instance_id: i.InstanceId }));
    return names;
};
...
async function start(args) {
    ...
    restart(ssh, args);
    return Object.assign(Object.assign({}, args[0]), { ssh: ssh });
}
...
async function generic_test(nservers, nclients, nactions, nkeys, crash) {
    ...
    if (instances.length < nservers)
        await startServers(nservers - instances.length);
