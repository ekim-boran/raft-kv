...
import { make_generic_test, create_raft, wait, one, nCommited, connect, disconnect, check_no_leader, check_one_leader, stop_test, send_RPC, restart, crash } from './util.js';
...
async function TestBasicAgree2B(config) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (nd > 0)
            throw "some have committed before Start()";
        let xindex = await one(config, i * 100, n, false);
        if (xindex != i)
            throw `got index ${xindex} but expected ${i}"`;
    }
}
...
async function TestBackup2B(config) {
    ...
    for (let i = 0; i < 50; i++)
        await start(config, leader, i + 50);
    await delay(500);
    ...
    for (let i = 0; i < 50; i++)
        await one(config, i, 3, true);
    let leader2 = await check_one_leader(config);
    let other = get(config, leader, 2);
    if (leader2 == other)
        other = get(config, leader2, 1);
    disconnect(config, other);
    for (let i = 0; i < 50; i++)
        await start(config, leader2, i + 50);
    await delay(500);
    for (let i = 0; i < n; i++)
...
async function TestFigure82C(config) {
    ...
    for (let iter = 0; iter < 1000; iter++) {
        ...
        if (Math.floor(Math.random() * 1000) < 100)
            ...
            await delay(Math.floor(Math.random() * 13));
