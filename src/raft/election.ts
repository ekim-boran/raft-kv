...
import { Raft, RequestVoteArgs, RequestVoteResponse, Roles, requestVoteArgs, requestVoteResponse } from "./types.js"


const requestVoteRPC = (to: string, args: RequestVoteArgs) =>
    SendRPC(to, "RequestVote", args).catch(e => requestVoteResponse("", -1, false))


function process_vote(raft: Raft, vote: RequestVoteResponse, votes: { accepted: number, rejected: number }, resolve: (ok: boolean) => void) {
    let majority = Math.floor(raft.peers.length / 2) + 1
    if (raft.role != Roles.Candidate) resolve(false);
    ...
    if (votes.accepted >= majority) resolve(true);
...
export async function StartElection(raft: Raft): Promise<void> {
    ...
    let ok: boolean = await new Promise(resolve => raft.peers.map(to => requestVoteRPC(to, args).then(vote => process_vote(raft, vote, votes, resolve))));
...
export function RequestVote(raft: Raft, { term, candidateId, logLength, logTerm }: RequestVoteArgs): RequestVoteResponse {
